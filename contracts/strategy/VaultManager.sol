...
/**
 * @title VaultManager
 * @dev Manages vault allocations across single and pair asset strategies with access control
 */
contract VaultManager is ReentrancyGuard, Pausable, AccessControl {
     ...
     */
    constructor(address _vaultRegister, address _manager, address _treasury, address _admin) {
        _grantRole(DEFAULT_ADMIN_ROLE, _admin);
        _grantRole(MANAGER_ROLE, _manager);
        vaultRegister = _vaultRegister;
        treasury = _treasury;
    }
    ...
    function executeAllocSingle(bytes32 key) external onlyManager nonReentrant {
        ...
        (StrategyKind kind, bool active) = IVaultRegistry(vaultRegister).strategies(alloc.strategy);
    ...
    function executeAllocPair(bytes32 key) external onlyManager nonReentrant {
        ...
        emit AllocationExecuted(key, alloc.allocatedA, alloc.allocatedB, wantA, wantB);
    }
    ...
    function _handleDeposit(
        ...
        (uint256 outA, uint256 outB) = IPairAssetStrategy(alloc.strategy).deposit(
            alloc.tokenA,
            diffA,
            alloc.tokenB,
            diffB
    ...
    function _handleWithdrawal(
        ...
        (uint256 outA, uint256 outB) = IPairAssetStrategy(alloc.strategy).withdrawByAmounts(
            alloc.tokenA,
            diffA,
            alloc.tokenB,
            diffB
    ...
    function addSingleAllocation(address token, uint256 cap, address stra) external onlyManager {
        (StrategyKind kind, bool active) = IVaultRegistry(vaultRegister).strategies(stra);
        if (!active) revert VAULT__STRATEGY_NOT_ACTIVE();
        if (kind != StrategyKind.SingleAsset) revert VAULT__INVALID_STRATEGY_KIND();
     ...
     */
    function harvestSingleAllocation(
        bytes32 key
    ) external onlyManager nonReentrant returns (address[] memory rewardTokens, uint256[] memory amounts) {
        SingleAllocation storage alloc = singleAllocs[key];
     ...
     * @return amounts Array of reward amounts
     */
    function harvestPairAllocation(
        bytes32 key
    ) external onlyManager nonReentrant returns (address[] memory rewardTokens, uint256[] memory amounts) {
        PairAllocation storage alloc = pairAllocs[key];
